    ...
    render() {
        return (
            <details className="details-overlay details-reset position-relative d-inline-block">
                <summary data-view-component="true" className="timeline-comment-action Link--secondary btn-link" aria-haspopup="menu" role="button">
                ...
                <detail-menu class="dropdown-menu dropdown-menu-sw show-more-popover color-fg-default anim-scale-in" style={{'width':185}} role="menu">
                    {this.props.actions.map((action, index) => {
                        ...
                        return (<Button title={action.title} classes={['btn-link', 'dropdown-item']} key={'d-action' + index} />);
                    })}
                </detail-menu>
            </details>
        );
